...
- **증상:** `/auth/callback`에서 세션 교환 실패, Google Chat에 동일 메시지 알림
- **원인:** OAuth 시작 시 PKCE verifier가 **브라우저 쿠키**에 남지 않았음. (다른 기기/브라우저에서 이어하기, 쿠키 삭제, 또는 서버에서만 OAuth를 시작해 `Set-Cookie`가 누락된 경우)
- **해결:** 로그인은 **클라이언트**에서 `signInWithOAuth`를 호출하도록 유지합니다. 운영 URL은 `NEXT_PUBLIC_APP_URL`이 실제 접속 도메인과 같아야 하며, Supabase Auth 리디렉션 URL에 `/auth/callback`이 등록되어 있어야 합니다.

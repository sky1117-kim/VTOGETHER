...
- **원인:** 1) 페이지가 새로고침되지 않아 이전 상태가 보임. 2) 이벤트 보상 설정이 비어 있음.
- **해결:**
  1. **페이지 새로고침:** 메인 페이지를 새로고침(F5)하거나 탭을 전환했다가 다시 돌아오면 최신 상태가 반영됩니다. (탭 전환 시 자동 갱신됨)
  2. **정책 확인:** 현재 정책은 보상 선택 없이 승인 시 자동 지급입니다.
  3. **이벤트 보상 확인:** 관리자 페이지에서 해당 이벤트의 보상이 등록되어 있는지 확인하세요. `event_rewards`에 V.Credit/V.Medal 수량이 없으면 지급되지 않습니다.

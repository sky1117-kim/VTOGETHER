...
- [x] **계정 삭제 기능** (2026.04.08): 마이페이지 본인 탈퇴(`/my`) + 관리자 계정 삭제(`/admin`) + 세아웍스 배치 동기화 시 퇴사자 자동 soft delete
- [x] **칭찬 수신자 사후 반영** (2026.04.08): `/admin/verifications` 승인 건에서 팀 칭찬 누락 수신자 보강 + 사후 보상 지급/알림 반영
- [x] **사후 반영 중복 방지/이력 표시** (2026.04.08): `045-point-transactions-backfill-recipient-unique.sql` + 누락 수신자 모달 이력(이미 반영/사후 반영 타임라인) 표시
...
- [x] 상점 주문 지급 체크(실물/알맹): `shop_orders.fulfilled_at` 기반 지급 완료 체크박스 `/admin/shop-orders` (2026.04.08)

...
- 기존 `api/actions/donation.ts`는 순차 업데이트 방식이라 중간 실패 시 **크레딧 차감만 반영**될 수 있었습니다.
- 마이그레이션 `042-donation-atomic-rpc.sql`의 `public.process_donation_atomic(...)`로 기부를 처리합니다.
- 이 함수는 **기부 내역, lot 차감, 거래 내역, 기부처 누적액, 사용자 포인트/누적기부/레벨업 메달**을 한 트랜잭션으로 실행합니다.
- 중간 단계에서 하나라도 실패하면 전체가 롤백되어, 사용자 잔액/기부 기록 불일치가 발생하지 않습니다.

## V.Honors (명예의 전당)

- 메인 화면 랭킹은 **현재 분기** `donations` 합계 기준입니다.
- `donations` 테이블은 RLS로 **본인 행만** 일반 클라이언트가 읽을 수 있으므로, 공개 랭킹 집계는 서버에서만 `createAdminClient()`(service role)로 읽어 **전 직원에게 동일한 순위**가 보이게 합니다. 개인 기부 내역 상세는 여전히 본인만 조회 가능합니다.
...
- **레벨 "선택" 필드 없음 (2026.04.06)**: 참여자가 DB에 목표 레벨(L1/L2/L3)을 저장하지 않습니다. 제출 UI의 L1~L3는 **월 합계 목표 안내**용이며, 심사 카드에는 **종목 1회 최소·L1~L3 임계값·현재 승인 누적 합·이 묶음만 승인 시 예상 달성 레벨**을 표시해 판단을 돕습니다(다른 대기 건은 예상 합에 넣지 않음).
...
- **푸터**: 개인정보처리방침 링크 자리 확보(현재 `#`). 실제 페이지 연동 시 href만 변경하면 됨.

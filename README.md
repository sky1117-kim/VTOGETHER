...
- **지급/적립 내역**: `/admin/point-grant` — **V.Credit·V.Medal 수동 지급** + 직원 전체 지급/적립/사용 거래 통합 조회(이름/이메일/사유 검색, 유형·재화·기간 필터, 페이지 이동) (2026.05.13)
- **이벤트 관리**: `/admin/events` — 목록, `/admin/events/new` — 등록 (보상: V.Credit/굿즈/커피쿠폰 복수 선택, 인증: 사진·텍스트·**객관식(선택지)**·숫자·동료선택+텍스트 + 직원 안내문)
...
- **칭찬 수신자 사후 반영**: `/admin/verifications` — 승인된 칭찬 챌린지에서 팀 칭찬 당시 계정이 없던 직원을 `누락 수신자 반영` 버튼으로 보강하고, 적립 내역/알림까지 반영 (2026.04.08)
- **사후 반영 중복 방지/이력**: `/admin/verifications` 누락 수신자 모달에서 이미 반영 인원·사후 반영 이력 확인 가능, DB 유니크 인덱스로 중복 지급 차단 (`045-point-transactions-backfill-recipient-unique.sql`) (2026.04.08)
...
- **상점 지급 체크**: `/admin/shop-orders` — 실물 굿즈/알맹 주문에 대해 지급 완료 체크(체크박스) 지원 (2026.04.08)
